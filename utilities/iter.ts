...
function iter<T>(generator: Generator<T>): Iter<T> {
	...
	iter.slice   = function(start: number, end: number): Iter<T> { return this.skip(start).take(end - start) }
	iter.at      = function(index: number): T   { return this.skip(index).next().value }
	iter.collect = function():              T[] { return [...this] }
	return iter
}
...
export function range(start_or_count: number, end?: number): Iter<number> {
	if (end == undefined)
		return infinite().take(start_or_count)
