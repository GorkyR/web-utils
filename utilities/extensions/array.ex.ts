...
Array.prototype.split = function(count) { return this.chunk( Math.ceil( this.length / count ) ) }
...
Array.prototype.interleave = function<T, I>(this: T[], ...separators: I[]): (T | I)[] {
	...
	for (let item of this.slice(1))
		interleaved.push( ...separators, item )
	return interleaved
}
...
Array.prototype.separate = function<T>(this: T[], predicate: (item: T, index: number) => boolean): [T[], T[]] {
	return this.reduce(([accepted, rejected], item, index) => 
...
Array.prototype.sort_by = function<T, K>(this: T[], keyer: (item: T) => K, ascending: boolean = true): T[] {
...
Array.prototype.sample = function<T>(this: T[], count?: number): T | T[] {
	...
	while (sampled.length < count)
		sampled.push( ...collection.splice( random(collection.length), 1 ) )
	return sampled
}
